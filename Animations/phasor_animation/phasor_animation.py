...
class ScopeRectCmbd:
    def __init__(self, ax, num_sigs, legend_list, xylabels, max_t=2 * fund_period, dt=Ts, plot_name='default_rect',
                 frame_size=num_sampls, debug=False):
        self.ax_r = ax
        self.ax_r.grid(True)
        self.dt = dt
        self.max_t = max_t
        ...
        # adding text
        self.time_text = self.ax_r.text(0.05, 0.9, '', transform=ax.transAxes)
    ...
    def update(self, emitted):
        ...
        if not pause:
            ...
            if self.debug is True:
                ...
                write_csv_array = [emitted[0].real, emitted[0].imag, x_index, x_index_wrapped]

                write_csv_dict = {'real val': write_csv_array[0],
                                  'imag val': write_csv_array[1],
                                  'x index': write_csv_array[2],
                                  'x wrapped index': write_csv_array[3],
                                  'plot name': self.plot_name,
                                  'frame': self.frame_count}

                print('csv array', write_csv_array)

                exists = os.path.exists(results_file_path)

                if exists:
                    with open(results_file_path, 'a', newline="") as f:
                        w = csv.DictWriter(f, write_csv_dict.keys())
                        w.writerow(write_csv_dict)
            ...
            if continuous:
                # this is not used at all, probably delete
                if self.t_data.size == 0:
            ...
            self.time_text.set_text(int(x_index_wrapped))

        # print(list(list_acs.list_acs.flatten_list_list(self.sig_lines_r)))
        return list(
            list_acs.flatten_list(
                self.sig_lines_r + self.sig_lines_r_cmbd + self.sig_lines_r_curr_pt + [self.time_text]))


class ScopePolarCmbd:
    def __init__(self, ax, num_sigs):
        ...
        # adding legend
        self.sig_lines_p_cmbd[0].set_label('Mag')
        self.sig_lines_p_cmbd[2].set_label('I')
        self.sig_lines_p_cmbd[3].set_label('Q')

        if self.polar_plot:
            self.ax_p.legend(bbox_to_anchor=(1.3, 1.3), loc="upper right")
        else:
            # self.ax_p.legend()
    ...
    def update(self, emitted):
        ...
        if not pause:
            ...
            # plt.draw()
        return list(list_acs.flatten_list(self.sig_lines_p + self.sig_lines_p_cmbd))
...
class Scope:
    def __init__(self, num_sigs, max_t=1 * fund_period, dt=Ts):
        self.rect_time = ScopeRectCmbd(ax_rect_cmbd, num_sigs, ['Real', 'Imag'],
                                       ...
                                       max_t / 10 ** Ts_pow10,
                                       dt / 10 ** Ts_pow10, plot_name='IQ_rect')
        ...
        self.rect_mag = ScopeRectCmbd(ax_rect_mag, num_sigs, ['Magnitude'], ['Freq', 'Amp [dB]'], max_t * w0,
                                      dt * w0, plot_name='Mag_rect')
        self.rect_phase = ScopeRectCmbd(ax_rect_phase, num_sigs, ['Phase'], ['Freq', 'Phase [Deg]'], max_t * w0,
                                        dt * w0, plot_name='Phase_rect')
    ...
    def update(self, emitted):
        ...
        # return list(list_acs.flatten_list(polars + lines_mag + lines_phase))

        return list(list_acs.flatten_list(lines_time + polars + lines_mag + lines_phase))

        # return list(list_acs.flatten_list(lines_time + polars))
